...
Consequently, the larger types use more memory and are slower than the smaller types, but have a larger range of possible values.
...
In situations where you only need to store positive values, unsigned integers are more efficient memory-wise (e.g. an 8 bit integer as twice the 0...255 rather than 0..127).
A common use of positive-only integers is when working with indexes in an array.
