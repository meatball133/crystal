...
describe "ParallelLetterFrequency" do
  pending "no texts" do
    input = [] of Int32
    expected = {} of Char => Int32
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end
  ...
  it "two texts with multiple letters" do
    input = ["ggh", "hhi"]
    expected = {"g" => 2, "h" => 3, "i" => 1}
    channel = Channel(Int32).new
    input.each do |text|
      spawn do
        ParallelLetterFrequency.calculateFrequencies(channel)
        channel.send(text)
      end
    end
  ...
  pending "ignore whitespace" do
    input = ["   ", "\t", "\r\n"]
    expected = {} of Char => Int32
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end

  pending "ignore punctuation" do
    input = ["!", "?", ";", ",", "."]
    expected = {} of Char => Int32
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end

  pending "ignore numbers" do
    input = ["1", "2", "3", "4", "5", "6", "7", "8", "9"]
    expected = {} of Char => Int32
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end

  pending "Unicode letters" do
    input = ["本", "φ", "ほ", "ø"]
    expected = {"本" => 1, "φ" => 1, "ほ" => 1, "ø" => 1}
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end

  pending "combination of lower- and uppercase letters, punctuation and white space" do
    input = [
    ...
    expected = {
      ...
      "k" => 6, "l" => 12, "m" => 7, "n" => 19, "o" => 22, "p" => 7, "r" => 17, "s" => 16, "t" => 30,
      "u" => 9, "v" => 2, "w" => 9, "y" => 4,
    }
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end

  pending "large texts" do
    input = [
    ...
    expected = {
      ...
      "q" => 8, "r" => 432, "s" => 700, "t" => 1043, "u" => 325, "v" => 111, "w" => 223, "x" => 7,
      "y" => 251,
    }
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end

  pending "many small texts" do
    input = [
      "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc",
      "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc",
      "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc",
      "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc",
      "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc", "abbccc",
    ]
    expected = {"a" => 50, "b" => 100, "c" => 150}
    ParallelLetterFrequency.calculateFrequencies(input).should eq expected
  end
end
